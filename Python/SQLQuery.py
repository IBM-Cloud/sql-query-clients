...
class SQLQuery():
    def __init__(self, api_key, instance_crn, target_cos_endpoint, target_cos_bucket, target_cos_prefix='', client_info=''):
    ...
    def get_result(self, jobId):
        ...
        result_location= "https://{}/{}?prefix={}jobid={}/part".format(self.target_cos_endpoint, self.target_cos_bucket, self.target_cos_prefix, jobId)
        ...
        if response.code == 200 or response.code == 201:
            ...
            print("Result object listing for job {} at {} failed with http code {}".format(jobId, result_location, response.code))
    ...
    def get_jobs(self):
        ...
        if response.code == 200 or response.code == 201:
            ...
            for job in job_list['jobs']:
                response = self.client.fetch(
                    "https://sql-api.ng.bluemix.net/v2-beta/sql_jobs/{}?instance_crn={}".format(job['job_id'], self.instance_crn),
                    method='GET',
                    headers= self.request_headers,
                    validate_cert=False)
                if response.code == 200 or response.code == 201:
                    ...
                    print("Job details retrieval for jobId {} failed with http code {}".format(job['job_id'], response.code))
                    break
        else:
    ...
    def sql_ui_link(self):
        if self.logged_on == False:
            print("You are not logged on to IBM Cloud")
            return
        print "https://sql.ng.bluemix.net/sqlquery/?instance_crn={}".format(urllib.unquote(self.instance_crn).decode('utf8'))
