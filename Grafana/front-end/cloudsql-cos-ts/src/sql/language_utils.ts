...
//# ------------------------------------------------------------------------------
import { CloudSQLMetricsMetadata } from "../types";
...
export function parseSelector(
  ...
  if (prefixClose > -1 && prefixClose > prefixOpen) {
    throw new Error(
      "Not inside selector, previous selector already closed: " + prefix
    );
  }
  ...
  if (suffixClose === -1) {
    throw new Error(
      "Not inside selector, missing closing brace in suffix: " + suffix
    );
  }
  if (suffixOpenIndex > -1 && suffixOpen < suffixClose) {
    throw new Error(
      "Not inside selector, next selector opens before this one closed: " +
        suffix
    );
  }
  ...
  selector.replace(labelRegexp, (label, key, operator, value) => {
    ...
    const valueEnd =
      labelOffset + key.length + operator.length + value.length - 1;
  ...
  const cleanSelector = labelKeys
    .map((key) => `${key}${labels[key].operator}${labels[key].value}`)
    .join(",");
...
export function expandRecordingRules(
  query: string,
  ...
  const expandedQuery = query.replace(
    rulesRegex,
    (match, pre, name, post) => `${pre}${mapping[name]}${post}`
  );
...
function addLabelsToExpression(
  expr: string,
  invalidLabelsRegexp: RegExp
): string {
 ...
 */
export function fixSummariesMetadata(
  metadata: CloudSQLMetricsMetadata
): CloudSQLMetricsMetadata {
  if (!metadata) {
    return metadata;
  }
  const summaryMetadata: CloudSQLMetricsMetadata = {};
