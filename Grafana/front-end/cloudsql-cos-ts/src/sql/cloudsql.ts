...
export const KEYWORDS: CompletionItem[] = [
  ...
  },
  {
    ...
    documentation:
      "An ETL that extract one or many columns from data stored in COS",
    //SELECT
...
export const TS_OPERATORS = [
  ...
  },
  {
    ...
    insertText:
      'USING TIME_SERIES_FORMAT(key="field_name", timetick="time_stamp", value="observation") IN ts',
    documentation:
  ...
  },
  {
    ...
    documentation:
      "This returns a time-series, by taking `sum` of values in each segment",
  },
  {
    label: "ts_seg_count",
    ...
    documentation:
      "This returns a time-series, by taking `count` of values in each segment",
  },
  {
...
const tokenizer = {
  ...
  function: new RegExp(
    `\\b(?:${FUNCTIONS.map((f) => f.label).join("|")})(?=\\s*\\()`,
    "i"
  ),
  "context-range": [
    {
  ...
  //},
  keyword: new RegExp(
    `\\b(?:${KEYWORDS.map((f) => f.label).join("|")})(?=\\b)`,
    "i"
  ),
  operator: [
    {
